...
v4: 12/02/2019:
    means e medians calcolati dentro has_folded. 
    NC per il threshodl deve essere 0.95*totalNC
v 3.1: 06/02/2019:
...
###################################################
def mediana(list_of_n_steps, total_n_of_runs):
    return list_of_n_steps[int(total_n_of_runs/2)-1] if total_n_of_runs % 2 == 0 else list_of_n_steps[int(total_n_of_runs/2)]
...
#    plt.hist(list_of_n_steps)    
    ...
    ################################/#
    # CALCULATE REAL FR
    time = np.mean(list_of_n_steps) if median == "mean" else mediana(list_of_n_steps, total_n_of_runs)
...
def folding_freq(protein_path, benchmark_rmsd, folded_runs_file):
    ...
    if folding_rate >= 0.5:
       ...
       median_time, median_rate, ln_median_rate  = "nan", "nan","nan"
