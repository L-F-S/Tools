...
def real_folding_rate(protein, protein_path, benchmark_rmsd,method, median = False):
    ...
    if median == False:
        ...
        real_FR = 1/np.median(list_of_n_steps)
    print("and the FR:", real_FR)
    ln_real_folding_rate = np.log(real_FR)
    ################################
    # PRINT INTO FILE
    if median == False:
        f=open("/home/lorenzo.signorini/tirocinio/log/real_folding_rates_"+method+"_bench", "a")
        f.write(protein+"\t\t"+str(benchmark_rmsd)+"\t\t"+str(real_FR)+"\t\t"+str(ln_real_folding_rate)+"\n")
    else:
        f=open("/home/lorenzo.signorini/tirocinio/log/median_real_folding_rates_"+method+"_bench", "a")
        f.write(protein+"\t\t"+str(benchmark_rmsd)+"\t\t"+str(real_FR)+"\t\t"+str(ln_real_folding_rate)+"\n")
...
def main():
    ...
    for protein in proteins:
         ...
         print("--------------------------------------------------\n\nPROCESSING", protein,':\n\n')
         method = "highest"
         bench_rmsd = benchmark_rmsd(protein, full_path_to_protein_folder, method)
         bench_rmsd = float(bench_rmsd)
    #     folding_rate(protein, full_path_to_protein_folder, bench_rmsd)
         real_folding_rate(protein, full_path_to_protein_folder, bench_rmsd, method, median=True)    
    return
