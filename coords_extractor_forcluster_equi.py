...
def extract_coordinates(protein, full_path_to_prot, do_nc=True, do_rmsd=True):
        ...
        if run.startswith("run"):
            ...
            for subfolder in os.listdir():
                ...
                if subfolder.startswith("native") and do_nc == True:
                     os.chdir(full_path_to_prot+"/native_contacts")
                     firstline = True
...
def main():
    ...
    for protein in proteins:
        if protein.startswith("1") or protein.startswith("2"):
             ...
             extract_coordinates(protein, full_path_to_protein_folder)
             print("DONE")
    return
